...
_bool CBounding_AABB::Collision(CCollider* pTargetCollider, _bool* pisCollision)
{
	...
	*pisCollision = false;
	...
	case CCollider::TYPE_AABB:
		*pisCollision = m_pAABB->Intersects(*(reinterpret_cast<CBounding_AABB*>(pTargetBounding))->Get_Bounding());
		break;
	case CCollider::TYPE_OBB:
		*pisCollision = m_pAABB->Intersects(*(reinterpret_cast<CBounding_OBB*>(pTargetBounding))->Get_Bounding());
		break;
	case CCollider::TYPE_SPHERE:
		*pisCollision = m_pAABB->Intersects(*(reinterpret_cast<CBounding_Sphere*>(pTargetBounding))->Get_Bounding());
		break;
	}
	return *pisCollision;
}
