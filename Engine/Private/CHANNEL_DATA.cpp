...
HRESULT CHANNEL_DATA::Make_ChannelData(aiNodeAnim* In_pChannel)
{
	...
	for (_uint i = 0; i < iNumKeyframes; i++)
	{
		if (iNumScalingKeys > i)
		{
			...
			KeyFrame.fTrackPosition = (_float)In_pChannel->mScalingKeys[i].mTime;
		}
		...
		if (iNumRotationKeys > i)
		{
			...
			KeyFrame.fTrackPosition = (_float)In_pChannel->mRotationKeys[i].mTime;
		}

		if (iNumPositionKeys > i)
		{
			memcpy(&KeyFrame.vPosition, &In_pChannel->mPositionKeys[i].mValue, sizeof(_float3));
			KeyFrame.fTrackPosition = (_float)In_pChannel->mPositionKeys[i].mTime;
		}
...
void CHANNEL_DATA::Bake_ReverseChannel(CHANNEL_DATA& Out_ChannelData, const _float In_fDuration)
{
	...
	for (_int i = (_int)Out_ChannelData.iNumKeyframes - 1; i >= 0; --i)
	{
		...
		KeyFrame.fTrackPosition = In_fDuration - KeyFrame.fTrackPosition;
