...
HRESULT CChannel::Initialize(const aiNodeAnim* pChannel)
{
	...
	for (size_t i = 0; i < m_iNumKeyFrames; i++)
	{
		...
		{
			//여기 주석 걸린것처럼 하면 터짐 순서가 w,x,y,z으로 되어 있음 쿼터니언은 그래서 하나하나 맞춰서 넣어줘야 함 
...
void CChannel::Invalidate_TransformationMatrix(_float fCurrentTrackPosition)
{
	...
	else
	{
		...
		//소스는 현재 Keyframeindex쪽이고 dest는 다음 keyFrameIndex임 
		...
		//선형보관할떄 현재 키프레임이 어디에 있는지 확인해야 하기 떄문에 레티오로 확인한다.
	...
	//지금까지 구했던 스케일 로테이션 포지션을 한번에 행렬로 만들어주는 개 꿀딱 함수
