...
HRESULT CVIBuffer_Dynamic_Terrain::Initialize_Prototype(const wstring& strHeightMapFilePath)
{
	m_iNumVerticesX = 100.f;
	m_iNumVerticesZ = 100.f;
...
void CVIBuffer_Dynamic_Terrain::Update_Terrain(_vector _vMousePos, _float _fRadious, _float _fPower,_int _iMode)
{
...
// CVIBuffer_Dynamic_Terrain* CVIBuffer_Dynamic_Terrain::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, _uint _iNumVerticesX, _uint _iNumVerticesZ)
// {
// 	CVIBuffer_Dynamic_Terrain* pInstance = new CVIBuffer_Dynamic_Terrain(pDevice, pContext);
// 
// 	/* 원형객체를 초기화한다.  */
// 	if (FAILED(pInstance->Initialize_Prototype( _iNumVerticesX, _iNumVerticesZ)))
// 	{
// 		MSG_BOX("Failed to Created : CVIBuffer_Dynamic_Terrain");
// 		Safe_Release(pInstance);
// 	}
