...
void CMagician::Priority_Tick(_float fTimeDelta)
{
...
HRESULT CMagician::Ready_Components_Origin(LEVEL eLevel)
{
	/* For.Com_Navigation */
...
HRESULT CMagician::Ready_PartObjects()
{
	CMagician_Body::BODY_DESC		BodyDesc = {};
	if (FAILED(Add_Body(TEXT("Prototype_GameObject_Magician_Body"), BodyDesc)))
		return E_FAIL;


	{
		CMagician_Weapon::WEAPON_DESC	WeaponDesc = {};
		if (FAILED(Add_Weapon(TEXT("Prototype_GameObject_Magician_Cane"), "ik_hand_r", WeaponDesc, TEXT("Weapon_R"))))
			return E_FAIL;
	}
