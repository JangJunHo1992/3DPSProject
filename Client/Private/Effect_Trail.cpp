...
void CEffect_Trail::Update(_float fTimeDelta, _matrix _ParentMatrix)
{
...
void CEffect_Trail::Set_OwnerDesc(_matrix _ParentMatrix)
{
	XMStoreFloat4x4(&m_pParentMatrix, _ParentMatrix);
}

HRESULT CEffect_Trail::Ready_Components(LEVEL eLevel)
{
	/* For.Com_Trail */
	if (FAILED(__super::Add_Component(eLevel, TEXT("Prototype_Component_VIBuffer_Trail"),
		TEXT("Com_Trail"), reinterpret_cast<CComponent**>(&m_pVIBuffer), &TrailDesc)))
		return E_FAIL;

	if (FAILED(__super::Add_Component(eLevel, TEXT("Prototype_Component_Shader_VtxPosTex"),
		TEXT("Com_Shader"), reinterpret_cast<CComponent**>(&m_pShader))))
		return E_FAIL;

	if (FAILED(__super::Add_Component(eLevel, TEXT("Prototype_Component_Texture_UVColorDiffuse"),
