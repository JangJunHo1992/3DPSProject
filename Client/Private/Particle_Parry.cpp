...
CParticle_Parry::CParticle_Parry(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{

}

CParticle_Parry::CParticle_Parry(const CParticle_Parry& rhs)
	: CGameObject(rhs)
{
	//m_pVIBufferCom
}

HRESULT CParticle_Parry::Initialize_Prototype()
{
	/* 원형객체의 초기화과정을 수행한다. */
...
HRESULT CParticle_Parry::Initialize(void* pArg)
{
...
void CParticle_Parry::Priority_Tick(_float fTimeDelta)
{

}

void CParticle_Parry::Tick(_float fTimeDelta)
{
...
void CParticle_Parry::Late_Tick(_float fTimeDelta)
{
...
HRESULT CParticle_Parry::Render()
{
...
HRESULT CParticle_Parry::Ready_Components(LEVEL eLevel)
{
	/* For.Com_Shader */
...
HRESULT CParticle_Parry::Bind_ShaderResources()
{
...
CParticle_Parry* CParticle_Parry::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CParticle_Parry* pInstance = new CParticle_Parry(pDevice, pContext);

	/* 원형객체를 초기화한다.  */
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CParticle_Parry");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CParticle_Parry::Clone(void* pArg)
{
	CParticle_Parry* pInstance = new CParticle_Parry(*this);

	/* 원형객체를 초기화한다.  */
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CParticle_Parry");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CParticle_Parry::Free()
{
	__super::Free();
