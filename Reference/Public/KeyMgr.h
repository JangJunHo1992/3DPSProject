...
public:
    static CKeyMgr* Create(HINSTANCE hInst, HWND hWnd);
    virtual void   Free();
...
private:
    ...
    //   KEY_STATE Get_KeyState(const int& iKey) { return m_mapInputKeys[iKey].eState; }
...
inline void CKeyMgr::Add_InputKey(INPUT_KEY_INFO* pInputKey)
{
}

inline void CKeyMgr::Remove_InputKey(INPUT_KEY_INFO* pInputKey)
{
}

bool CKeyMgr::Key_Pressing(const int& iKey)
{
    if (Get_DIKeyState(iKey) & 0x8000)
        return true;
...
bool CKeyMgr::Key_Down(const int& iKey)
{
    ...
    if (!m_bKeyState[iKey] && (Get_DIKeyState(iKey)))
        return true;
...
bool CKeyMgr::Mouse_Pressing(const MOUSEKEYSTATE& iMouse)
{
    ...
    {
        if (Get_DIMouseState((MOUSEKEYSTATE)iMouse))
            return true;
    }
    else if (iMouse >= DIM_MB && iMouse < DIM_END)
    {
        switch ((MOUSEKEYSTATE)iMouse)
        {
        case DIM_MWU:
        {
            if ((Get_DIMouseMove(DIMS_Z) > 0L))
                return true;
            break;
        }
        case DIM_MWD:
...
bool CKeyMgr::Mouse_Down(const MOUSEKEYSTATE& iMouse)
{
    if (iMouse >= 0L && iMouse <= DIM_MB)
    {
        if (!m_bMouseState[iMouse] && Get_DIMouseState((MOUSEKEYSTATE)iMouse))
            return true;
    }
    else if (iMouse >= DIM_MB && iMouse < DIM_END)
    {
        ...
        case DIM_MWU:
        {
            if (!m_bMouseState[iMouse] && (Get_DIMouseMove(DIMS_Z) > 0L))
                return true;
            break;
        }
        case DIM_MWD:
